...
const getSensores = async (req, res) => {
    try {
        ...
        const result = await connection.query("SELECT * FROM sensor");
        //const result = await connection.query("SELECT id, temperatura, humedad, fecha_actual FROM sensor");
        //console.log(result);
        res.json(result);
    } catch (error) {
        res.status(500);
...
/*const getLanguage = async (req, res) => {
    try {
        const { id } = req.params;
        const connection = await getConnection();
        const result = await connection.query("SELECT id, temperatura, humedad, fecha_hora FROM language WHERE id = ?", id);
        res.json(result);
    } catch (error) {
        res.status(500);
        res.send(error.message);
